...
const actions = {
    async getTagList({ commit, state }, config) {
        ...
        const {data, code} = await api.backendGetTagList({...config});
        if (data && code === 200) {
            commit(mutationTypes.RECEIVE_TAG_LIST, {
                ...config,
                ...data
            });
        }
    ...
    changeDataLocal({ commit, state }, data) {
        commit(mutationTypes.CHANGE_TAG_LOCAL, data);
    }
};

const mutations = {
    [mutationTypes.RECEIVE_TAG_LIST](state, {list, hasNext, hasPrev, page, path, total, totalPage}) {
        if (page === 1) {
            list = [].concat(list);
        } else {
    ...
    [mutationTypes.CHANGE_TAG_LOCAL](state, data) {
